...
public abstract class RetrieveCancelledDocumentsJpa implements RetrieverOutputBoundary {
    ...
    public List<Map<String, Object>> retrieve() throws Exception {
        var documents = findCancelledDocuments(initialDocumentNumber);
    ...
    private List<Map<String, Object>> convertToMap(List<Object[]> documents) {
        ...
        for (int i = 0; i < documents.size(); i++) {
            ...
            try {
                ...
                documentMap.put("anulado", dateToLocalDate((Date) document[16]));
                result.add(documentMap);
            } catch (Exception e) {
                logger.error("No se pudo procesar el documento anulado en la iteracion " + i + " con el id " + document[0]);
